      ...
      function(callback) { // storage
        clearFolder(that.runConfig.paths.tmpFolder, function(err) {
          setTmpFolder(that.runConfig.paths.tmpFolder, function(err) {
            ...
            that.store = levelup(that.runConfig.paths.storageFolder);
            callback(null);
          });
        });
      },
...
var setRunConfig = function(server, cb){
  server.runConfig = {
    paths: {},
    storageFileName: server.config.storageFileName||  'storage'
  };
  server.runConfig.paths.tmpFolder = global.__projectPath+(server.config.path?'/':'')+server.config.path+'/tmp';
  server.runConfig.paths.storageFolder = global.__projectPath+(server.config.path?'/':'')+server.config.path+'/tmp/'+server.runConfig.storageFileName;
  cb(null);
};
